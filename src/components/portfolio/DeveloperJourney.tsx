...
const journeyData = [
  ...
  },
  {
    ...
    period: '2025 - 2026',
    description:
      'Top 117 Finalist in the HSBC Hackathon (selected from 30,000+ applicants). Actively building AI-integrated solutions for competitive hackathons like HackWithAI.',
    icon: Trophy,
  ...
  },
  {
    ...
    description:
      'Orchestrated the development of DealHunter, a Python web scraper with a Streamlit dashboard, achieving 15% faster filtering. Built a Creative Jobs Portal using Selenium automation.',
    icon: Code,
  ...
  },
  {
    ...
    period: '2022 - 2026',
    description:
      'Maintaining a strong academic foundation with a 9.1 CGPA while continuously building hands-on web applications.',
    icon: GraduationCap,
...
export function DeveloperJourney() {
  // ── FIX: useScroll's `target` ref must point to a POSITIONED element.
  // We attach the ref to an inner div that already has `relative` via Tailwind,
  // which satisfies framer-motion's scroll-offset calculation and silences the
  // "non-static position" console warning.
  ...
  return (
    <Section
      ...
      description="A timeline of my growth, from education to real-world impact."
    >
      {/*
        ── FIX: `relative` is already applied below via Tailwind, but we also
        add an inline style to be completely explicit — some framer-motion
        versions inspect the computed style and miss Tailwind classes that
        haven't been applied yet at mount time.
      */}
      <div
        ref={containerRef}
        className="relative max-w-4xl mx-auto py-10 px-4"
        style={{ position: 'relative' }}   // ← explicit so framer-motion sees it immediately
      >
        ...
        <div className="flex flex-col gap-12 relative z-10">
          {journeyData.map((item, index) => {
            ...
            return (
              ...
              >
                {/* Desktop spacer (pushes card to correct side) */}
